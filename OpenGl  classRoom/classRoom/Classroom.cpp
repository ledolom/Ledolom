...
EyePoint myEye;//相机在世界坐标的位置
EyePoint vPoint;//相机镜头对准的物体在世界坐标的位置
EyePoint up;//相机向上的⽅向在世界坐标中的⽅向
GLfloat vAngle = 0;
//载入位图作为纹理的相关函数
...
void drawscence()
{
	...
	/*glBindTexture(GL_TEXTURE_2D, texairback);
	glBegin(GL_QUADS);
	glTexCoord2f(0.0f, 0.0f); glVertex3f(37.0f, 0.0f, -26.0f);
	glTexCoord2f(0.0f, 1.0f); glVertex3f(37.0f, 15.0f, -26.0f);
	glTexCoord2f(1.0f, 1.0f); glVertex3f(37.0f, 15.0f, -29.0f);
	glTexCoord2f(1.0f, 0.0f); glVertex3f(37.0f, 0.0f, -29.0f);
	glEnd();*/
...
int main(int argc, char* argv[])
{
	...
	texceiling = load_texture("sky3.bmp");
	texdoor = load_texture("door.bmp");
	texfloor = load_texture("tdesk.bmp");
	texbackwall = load_texture("backwall1.bmp");
	...
	texcuc = load_texture("SUT.bmp");
	texcsc = load_texture("MSA.bmp");
	texlmy = load_texture("img.bmp");
